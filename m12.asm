...
// org $8F0F226; bl copy_battle_line_to_ram  // [M1-only cleanup] disabled (required code stubs removed)

// add a space between the enemy name and the suffix letters if there are multiple enemies
// org $8F0F2DA; bl add_space_to_enemy_name  // [M1-only cleanup] disabled (required code stubs removed)

// only undo auto-indenting if it's battle text
// org $8F0C088; bl possibly_ignore_auto_indents  // [M1-only cleanup] disabled (required code stubs removed)
...
// org $8F0CAE4; bl choose_text_window_type  // [M1-only cleanup] disabled (required code stubs removed)
org $8F0AE48; dd wide_text_box
org $8F7E100
wide_text_box:
   incbin m1_window_wide_text_box.bin
// org $8F0CB1E; bl save_line_number_a  // [M1-only cleanup] disabled (required code stubs removed)
// keep game from updating character status on top of dialog boxes
org $8F0B01C; nop; nop;

// let capsule items be "swallowed" instead of "drank"
// org $8F08576; bl swallow_item  // [M1-only cleanup] disabled (required code stubs removed)
// org $8F08586; bl swallow_item  // [M1-only cleanup] disabled (required code stubs removed)
// org $8F08596; bl swallow_item  // [M1-only cleanup] disabled (required code stubs removed)
// org $8F085A6; bl swallow_item  // [M1-only cleanup] disabled (required code stubs removed)
// org $8F085B6; bl swallow_item  // [M1-only cleanup] disabled (required code stubs removed)
...
// org $8F04FCE; bl choose_yes_no_size  // [M1-only cleanup] disabled (required code stubs removed)
...
// org $8F1258E; bl increaseexp  // [M1-only cleanup] disabled (required code stubs removed)
// org $8F10350; bl increasemoney  // [M1-only cleanup] disabled (required code stubs removed)
// org $8F09698; bl lowerencounterrate  // [M1-only cleanup] disabled (required code stubs removed)
// //org $8F0E5E0; bl increase_offense  // turns the Easy Ring into the prank Hard Ring  // [M1-only cleanup] disabled (required code stubs removed)
...
org $8F633EC; incbin m1_gfx_produced_by_a.bin
org $8F6350C; incbin m1_gfx_produced_by_b.bin
// org $8F0D5D0; bl producescreen1; b $8F0D5E8  // [M1-only cleanup] disabled (required code stubs removed)
// org $8F0D63C; bl producescreen2; b $8F0D664  // [M1-only cleanup] disabled (required code stubs removed)
org $8F0D66E; nop
org $8F0D676; nop
